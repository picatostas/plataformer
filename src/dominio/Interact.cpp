...
#include "pang\CoordinadorJuego.h"
#include <iostream>
using namespace std ;
...
{ 
	...
	if(h.pos.x>xmax)h.pos.x=xmax; 
	if(h.pos.x<xmin)h.pos.x=xmin; 
	if(h.pos.y>ymax)h.pos.y=ymax;
	if(h.pos.y<=ymin)
	{
		h.pos.y=ymin;
		h.vel.y=0;
...
bool Interact::Rebote(Esfera &e,Pared p)
{
	...
	{ 
		...
		e.vel=v_inicial-dir*2.0*(v_inicial*dir); 
		e.pos=e.pos-dir*dif; 
		return true; 
	} 
	return false;
...
void Interact::Rebote(Esfera &e, Caja c)
{
	...
	if (flag_suelo==true)
	{
		...
		if(e.pos.x>xmax)e.pos.x=xmax; 
		if(e.pos.x<xmin)e.pos.x=xmin;
	}
	...
	if (flag_pared_izq==true)
	{
		...
		if(e.pos.y>ymax)e.pos.y=ymax; 
		if(e.pos.y<ymin)e.pos.y=ymin;
	}
